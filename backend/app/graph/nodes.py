...
class AttractionDetails(BaseModel):
    ...
    cost: float = Field(description="Estimated cost per person in USD (0 for free attractions)")
...
class BudgetAgentOutput(BaseModel):
    hotel_costs: float = Field(description="Sum of hotel costs (calculated for the chosen hotel classification multiplied by the number of nights, which is days - 1)")
    activity_costs: float = Field(description="Sum of all attraction costs for the trip")
    food_costs: float = Field(description="Estimated total cost of meals for the duration of the trip")
    transport_costs: float = Field(description="Estimated total cost of local transit (metro, taxis) for the trip")
    total_cost: float = Field(description="Sum of hotel_costs, activity_costs, food_costs, and transport_costs")
...
def get_llm(provider: str, custom_key: Optional[str] = None):
    # Retrieve key: custom key overrides environment variables
...
def planner_agent(state: PlanState) -> PlanState:
    ...
    try:
        ...
        prompt = (
            ...
            f"with a budget of ${state['budget']} USD.\n"
...
def hotel_agent(state: PlanState) -> PlanState:
    """Hotel Agent: Finds suitable lodging options matching the destination and budget."""
    if state.get("error"):
        return state
    ...
    try:
        ...
        prompt = (
            ...
            f"The overall trip budget is ${state['budget']} USD.\n"
            ...
            f"Estimate the price per night in USD and calculate the total cost for {state['days'] - 1} nights (since it is a {state['days']}-day trip)."
        )
        
        response = structured_llm.invoke(prompt)
        # Convert Pydantic output to list of dicts and calculate totals
        hotels_list = []
...
def attractions_agent(state: PlanState) -> PlanState:
    """Attractions Agent: Researches and builds daily activity itineraries."""
    if state.get("error"):
        return state
    ...
    try:
        ...
        prompt = (
            ...
            f"Tailor the recommendations to the daily geographic focus areas and the general budget: ${state['budget']} USD."
        )
        
        response = structured_llm.invoke(prompt)
        
        # Format the daily plans into dicts
        daily_itinerary = []
...
def budget_agent(state: PlanState) -> PlanState:
    """Budget Agent: Synthesizes expenses, chooses the best hotel fitting the budget, and adjusts costs."""
    if state.get("error"):
        return state
    ...
    try:
        ...
        # Extract inputs for the budget compiler
        hotels = state["hotels"]
        itinerary = state["daily_itinerary"]
        
        hotels_str = "\n".join([f"- {h['name']} ({h['location']}): ${h['price_per_night']}/night (Total: ${h['estimated_total']})" for h in hotels])
        ...
        prompt = (
            ...
            f"Evaluate the budget for a {state['days']}-day trip to {state['destination']} with a user budget of ${state['budget']} USD.\n\n"
            f"Available Hotel Options (for {state['days']-1} nights):\n{hotels_str}\n\n"
            f"Activity Costs (excluding hotels and meals): ${activities_cost:.2f} USD\n"
            f"Estimated Meal Costs (from suggested restaurants): ${food_cost_estimate:.2f} USD\n\n"
            f"Your Task:\n"
            ...
            f"3. If total cost exceeds the user's budget (${state['budget']}), explain what swaps or recommendations you would suggest (e.g. choosing the Budget hotel option instead, suggesting free walking tours, or eating street food).\n"
            f"4. Determine status: 'within_budget' or 'over_budget'. If over budget, make reasonable adjustments to total costs to represent a realistic, optimized budget suggestion."
        )
